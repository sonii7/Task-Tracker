...
export default function TaskTracker() {
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get('http://127.0.0.1:3088/tasks');
  ...
  const handleCreateTask = async () => {
    try {
      await axios.post('http://127.0.0.1:3088/tasks', { title: taskTitle, description: taskDescription });
      fetchTasks();
      setTaskTitle('');
      setTaskDescription('');
  ...
  const handleUpdateTask = async (taskId) => {
    try {
      await axios.put(`http://127.0.0.1:3088/tasks/${taskId}`, { title: taskTitle, description: taskDescription });
      fetchTasks();
  ...
  const handleDeleteTask = async (taskId) => {
    try {
      await axios.delete(`http://127.0.0.1:3088/tasks/${taskId}`);
      fetchTasks();
            ...
            <div key={task._id} style={{margin:'20px'}}>
              <Card className="bg-dark text-white" style={{ width: '18rem', height: '15rem' }}>
                <Card.Body>
                  ...
                  <Button style={{margin:'5px'}} variant="primary" onClick={() => handleEditTask(task._id)}>Edit</Button>
                  <Button style={{margin:'5px'}} variant="primary" onClick={() => handleUpdateTask(task._id)}>Update</Button>
                  <Button style={{margin:'5px'}} variant="primary" onClick={() => handleDeleteTask(task._id)}>Delete</Button>
                </Card.Body>
              </Card>
            </div>
          ))}
